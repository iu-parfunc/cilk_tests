...
Matrix copy_matrix(int depth, Matrix a)
{
    ...
    r11 = cilk_spawn copy_matrix(depth, a->child[_11]);
    cilk_sync;
...
Matrix cholesky(int depth, Matrix a)
{
    ...
    if (!a10) {
         ...
         a11 = cilk_spawn cholesky(depth, a11);
         cilk_sync;
    } else {
         a00 = cilk_spawn cholesky(depth, a00);
         cilk_sync;
         ...
         a10 = cilk_spawn backsub(depth, a10, a00);
         cilk_sync;
         assert(a10);

         a11 = cilk_spawn mul_and_subT(depth, 1, a10, a10, a11);
         cilk_sync;
         assert(a11);
...
int main(int argc, char *argv[])
{
    ...
    * nice big fat diagonal for Cholesky
    ...
    A = set_matrix(depth, A, i, i, 1.0);
     ...
     R = cilk_spawn copy_matrix(depth, A);
     cilk_sync;
     ...
     A = cilk_spawn mul_and_subT(depth, 1, R, R, A);
     cilk_sync;
     error = mag(depth, A);
