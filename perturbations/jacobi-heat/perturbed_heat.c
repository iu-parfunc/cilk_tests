...
int divide(int lb, int ub, double **new, double **old, int mode, int timestep)
{
  int l, r;

  if (ub - lb > leafmaxcol) {
    l = cilk_spawn divide(lb, (ub + lb) / 2, new, old, mode, timestep);
    r = cilk_spawn divide((ub + lb) / 2, ub, new, old, mode, timestep);
    cilk_sync;
    return (l + r);
  } else {
    switch (mode) {
...
int heat(void)
{
  ...
  l = cilk_spawn divide(0, nx, new, old, ALLC, 0);
  cilk_sync;
  /* 
  ...
  /* Initialization */
  l = cilk_spawn divide(0, nx, new, old, INIT, 0);
  cilk_sync;
  ...
  for (c = 1; c <= nt; c++) {
    ...
    perturb(prob, c, length);
    l = cilk_spawn divide(0, nx, new, old, COMP, c);
    cilk_sync;
  }
...
read_heatparams(char *filefn)
{
  ...
  if (l != 9)
    printf("\n Warning: fscanf errno %d", 2);
  fclose(f);

}

char *specifiers[] = { "-p", "-l", "-g", "-nx", "-ny", "-nt", "-xu", "-xo", "-yu", "-yo", "-tu", "-to", "-f", "-benchmark", "-h", 0};
int opt_types[] = {LONGARG, LONGARG, INTARG, INTARG, INTARG, INTARG, DOUBLEARG, DOUBLEARG, DOUBLEARG, DOUBLEARG, DOUBLEARG, DOUBLEARG, STRINGARG, BENCHMARK, BOOLARG, 0 };
...
int main(int argc, char *argv[])
{
  ...
  get_options(argc, argv, specifiers, opt_types, &prob, &length, &leafmaxcol, &nx, &ny, &nt, &xu, &xo, &yu, &yo, &tu, &to, filename, &benchmark, &help);

#ifdef CILK_BLOCK
  cilk_io_init();
#endif

  printf("prob = %lu\t length = %lu\n", prob, length);
